...
* 加大选择文字的判定触摸范围，容易点空。
#### 漫画模式
* （已实现）增加了一个漫画模式，当前书源注释里添加comic或者分组名包含漫画关键词时，该书源中的书籍启动漫画模式，该设置下就可以根据喜好去掉padding，默认翻滚翻页等，漫画模式下阅读时固定黑底白字，长按无效。
* （已实现）漫画模式切非漫画模式的书，会重新刷新书目，有时间看看能不能优化。漫画模式在加载内容前就设定好。先加书架的书，后面各种情况进入书页都支持
* （已实现）看漫画时跳转目录很容易断言崩溃。原版就有这个问题，怀疑是多个协程做loadContent没保护好。---Cache清除没加保护。
...
* （已实现）一些字体的标点是居中显示的。需要准确截取到起始x。以获得更好的压缩效果。暂时不知道怎么截取，试一下textpaint的几个接口，实在不行先做成按照不同的界面配置设置压缩修正值。----使用getTextBounds可以准确获取字符边界。
#### 阅读配置
...
#### 底栏
* （已实现）底栏图标支持渐变色，更换四大金刚--换成了微读风格
#### 阅读记录
* （已实现）阅读记录超过5分钟以上才显示，支持取消
* （已实现）默认按阅读顺序排序
* （已放弃）书籍读完到最后一页，出现选择界面，选择为已读，或者选择为允许更新。书籍默认不允许更新。读完后增加三个框选，是否标记已读，是否移除书架，是否更新。
* （已放弃）增加想读，每次不在书架且不在想读的书，置成想读。当放入书架，去除想读。
* （不需要）阅读记录上传WEBDAV
* （已实现）增加是否有书签的接口。
* （已实现）通过阅读记录入口可以进入已读、书签。
* （已实现）阅读记录增加封面url，书籍url，是否已读状态。是否有书签。
* （已实现）阅读记录里直接点击到书籍页。
* （已实现）阅读时间增加监控，如果30S没有点击事件就不要继续累积阅读时间了。
* （已实现）记录每天的阅读时间。在书籍详情展示。
* （已实现）书籍内部本次阅读时间改为当天阅读时间
* （已实现）阅读记录增加最近阅读展示
* （已实现）阅读时间在书籍信息中展示
...
* （已解决）Web端支持同步阅读时间（需要看看现在怎么实现的）
* Web支持漫画模式，可以设置line-height=0
* Web下支持pos索引。索引可以同手机同步
## 其他
#### 备份恢复
* （已实现）修复恢复无法恢复部分配置问题
* （已实现）首次打开软件即提示开启存储空间
...
* （已实现）换源界面显示当前的书源和分组
* （已实现）换源界面支持书源管理选择
* （已实现）优化换源处理的线程逻辑和刷新逻辑。现在刷新完就真的处理完了。重新刷新RecycleView也会刷。数据库只清楚本分组的。
* （已实现）增加了一个搜索延时统计，方便找出劣质书源，先简单做一下，后面细化。
* （不需要）换源界面点击标题和作者可以进入搜索界面
* （已实现）增加选项忽略作者
* 美化搜索延时展示界面
#### 书签
* 有书签直接在段落末位显示可点击的小图标
* 独立的书签内容展示页。效果美化。
#### EPUB书籍
* （已实现）修复同一resource里有多个章节的书籍的兼容性。修复卫斯理全集内容错误
* （已实现）修复一个章节分散到多个resource的书籍的兼容性。修复雪中悍刀行网文多看版的内容缺失。现在使用的是数据库存的子章节。不知道有没有更简单的方法。
* 检查加载章节部分的代码。优化兼容性
* 支持多级目录和折叠目录
* 修复妖艳人生目录解析失败--这本书是opf的package头打的有问题，PC上改就可以看了，但是很多阅读器不修复也能打开。不知道这些阅读器是重修了opf，还是直接把包里所有的资源重扫了。重修头如果懂相关语法针对这个错误应该也很简单的，但这个修复有通用性吗。这块现在完全还没有概念。最好找个兼容性强的epub开源代码学习下。
* 测试其他本地EPUB书籍
* 引用弹注展示
* 支持CSS
* 增加段落笔记功能 
