...
#### 1月2日
沉浸式阅读配置的想法基本上完成了主要内容了。从拉代码以后最想改的就是这一块，因为阅读配置是最常用的一个界面，原设计这块没有仔细打磨。前前后后试了好几次。主要是涉及的控件、BAR、图标和相关的颜色配置太散了，看起来都是图标，结果接口实现不一样。通过各种尝试、查资料、读代码，终于各个击破都改掉了。

整体思路就是协调上下窗口风格设计，精细调整UI、配色、间距、分割等等。改的时候才发现，随便哪个细节调一调，颜值都能升一升。第一次搞这个，高度学习了新厚墨和微信读书，好多图标都是拿来主义，反正只有自己用，哈哈！改的时候强烈感觉新厚墨应该有很多参考微信读书的设计。但还是微信读书设计更完美，如阅读配置底栏4大金刚设置时常驻，且除目录页外其他三个的高度都是一致的。有很多类似细节，非常有设计美感。不去学习平常根本发现不了。另外新厚墨的纤细图标风非常赞。真是不亚于大厂设计了。

改的过程中学习了不少UI相关的知识。发现legado真是个挺庞大的工程。每一个控件，颜色，按压效果都有需要代码实现。主题商店各种配色如何在每一处体现、各个颜色如何互斥等等，各种细节都有考虑，大佬代码里做了这么多东西，真的很棒！

还有字体设置界面和更多设置界面没有改，考虑下要不要改。因为改的时候尽量不想破坏原有代码和设计，改动太大的话后面合主代码次次conflicts太烦了。最近参考书刚刚看完RecycleView，准备学Fragment和Material Design了。下一步看看是不是做做阅读统计这块。
# 优化列表 
## 阅读界面
#### 阅读界面
...
* 修改可生成多个Activity的设计。
* 加大选择文字的判定触摸范围，容易点空。
* 增加一个漫画模式，比如书源注释里添加comic时，该书源中的书籍启动漫画模式，该模式下阅读时默认采用翻滚翻页，默认padding为0，默认长按无效。
#### 排版
* （已实现）行首行尾标点溢出处理
* （已实现）冒号等支持在行尾
