...
* （已实现）默认按阅读顺序排序
* **书籍读完到最后一页，出现选择界面，选择为已读，或者选择为允许更新。书籍默认不允许更新。**
* 读完后增加三个框选，是否标记已读，是否移除书架，是否更新。
* 增加想读，每次不在书架且不在想读的书，置成想读。当放入书架，去除想读。
* 增加是否有书签的接口。
* 通过阅读记录入口可以进入已读、想读、书签。
* 阅读记录增加封面url，书籍url，是否已读状态。是否有书签。
* 阅读记录里直接点击到书籍页，找不到书籍页进入搜索页。
