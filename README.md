...
#### 2月21日
修复了一下epub的兼容性。手头的epub书基本搞定了。（还有一种opf头不对的还在看怎么处理）。增加了去除图片去除标题去除注解的配置。加速了打开epub书籍的速度。现在纠结要不要支持CSS。市面有大量多看精排书，想支持到类似的效果看下框架应该能实现。但是完美主义强迫症发作，web上读不到本地图片效果难统一，legado的本行搜书效果也对比下去了。不如做几个内置的精排模板，什么书都能套用的。

然后增加了web上的阅读记录时间统计。增了web上的阅读进度精准同步。增加了web上的书签添加功能。增加了同步和保存按钮。web刚开始接触，需要抽时间学习下html和JavaScript、还有vue。完善了当前的进度同步逻辑，在手机上点同步，可以看到云的进度，网页的进度，本机上次的进度，如果不小心点错可以回到本地上次进度记录，这块春节反复优化了好久，基本上舒服了。
# 优化列表 
## 阅读界面
#### 漫画模式
...
* （已实现）增加对epub书籍去除CSS的配置按钮。去除正文标题，去除弹注和标注，去除图片。
* （已实现）增加epub书籍缓存。加速每次打开epub书籍的速度。
* **设置去除CSS时刷新当前epub书籍的当前章缓存。增加一个清除所有缓存的按钮。**
* **书籍导入窗口支持按照文件添加顺序排序。
* **书籍导入窗口返回键到当前目录就返回了，不需要到达最顶层目录
* 支持多级目录和折叠目录
* 测试其他本地EPUB书籍
* 引用弹注展示
* 支持CSS
* 增加段落笔记功能 
* 修复掌上书苑部分书籍解析失败--opf的package头打的有问题，PC上改就可以看了，但是很多阅读器不修复也能打开。不知道这些阅读器是重修了opf，还是直接把包里所有的资源重扫了。重修头如果懂相关语法针对这个错误应该也很简单的，但这个修复有通用性吗。这块现在完全还没有概念。最好找个兼容性强的epub开源代码学习下。
## WEB
* （已实现）Web端支持同步阅读时间
* （已实现）开放右侧滚动条
* （不需要）Web支持两端对齐
* （已实现）Web下支持pos索引。索引可以同手机同步
* （已实现）Web支持默认封面，与本地默认封面保持一致
* （已实现）web支持书签-选择文本，点击键盘m
* （已实现）参数精确的chapterPos。目前误差在5个字符以内。懒得继续查哪里多出来了
* （以实现）修复web看书缩进不对问题。
* 完善web书签的体验：右键菜单选择，弹出填写个人内容。
* Web支持漫画模式，可以设置line-height=0
## 替换净化
* **在书籍阅读界面的替换净化，打开只显示全局净化规则和当前数据的规则。不显示其他书籍的规则。（可以增加一个菜单，是否显示其他书籍的规则）
* **规则排序：本书籍规则排序在前，全局规则排序在后。
## 目录存储
* 修改当前目录存储逻辑，支持把本地缓存目录的内容存储到自定义目录中。设置基准目录。备份放到backup下。数据放到data下。下次重装软件也就不需要下书籍缓存了。
* 增加清除过期缓存的功能。如超过1个月的缓存就不要了。
## 音频
* 修复点击状态栏返回书目不对问题
* 增加单曲循环功能
