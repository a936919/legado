...
* （已实现）修复目录第一章前的内容不显示问题。
* （已实现）处理正文中自带的标题分为2段的情况时不显示该标题。
* （已实现）设置更改CSS配置时刷新当前epub书籍的当前章缓存。
* （已实现）书籍导入窗口返回键到当前目录就返回了，不需要到达最顶层目录
* （已实现）书籍导入窗口支持按照文件添加顺序排序。
* （已实现）支持image格式封面在打开书籍第一页显示。
...
## 替换净化
* （已实现）在书籍阅读界面的替换净化，打开只显示全局净化规则和当前数据的规则。不显示其他书籍的规则。
* （已实现）规则排序：当只显示局部替换时，按照更新时间排序
* **界面显示当前的范围**
* **替换净化增加选择当前限定范围**
## 目录存储
