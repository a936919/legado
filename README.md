...
#### 2月4日
修复了一下epub的兼容性。手头的epub书基本搞定了。增加了去除图片去除标题去除注解的配置。现在纠结要不要支持CSS。市面有大量多看精排书，想支持到类似的效果看下框架应该能实现。但是完美主义强迫症发作，web上读不到本地图片效果难统一，legado的本行搜书效果也对比下去了。不如做几个内置的精排模板，什么书都能套用的就更完美了。
